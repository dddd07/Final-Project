...
//          BoardEntity board = brepo.searchRStatus(rev.getBNum(),rev.getUserId(),1);
//
//          if(board == null) {
        System.out.println("REV User ID: " + rev.getUserId());
        ...
        try {
            ...
            rrepo.save(entity);
            brepo.saveRStatus(entity.getBNum(), entity.getUserId());
...
//        }
    ...
    public ModelAndView RView(int REVNum) {
        ...
        // 로그인 여부 체크
        if (loginId == null) {
            mav.setViewName("redirect:/mLoginForm"); // 로그인 페이지로 리다이렉트
            return mav;
        }
