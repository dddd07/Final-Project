...
public class UserService {
    ...
    public ModelAndView mdJoin(UserInfoDTO user) {
        ...
        try {
            ...
            session.invalidate();
            mav.setViewName("redirect:/");  // 수정 후 리다이렉트
        } catch (Exception e) {
...
//
//        // 관리자 아이디와 비밀번호 비교
//        if (user.getUserId().equals("admin") && user.getUserPw().equals("1234")) {
//            // 관리자 로그인 성공
//            session.setAttribute("loginId", "admin");  // 관리자로 세션 처리
//            session.setAttribute("loginName", "관리자");
//            session.setAttribute("adminName", "admin");
//
//            // 관리자 페이지로 리디렉션
//            mav.setViewName("redirect:/");  // 관리자 페이지로 리디렉트
//            return mav;
//        }
//
//        // (1) 아이디가 존재하는지 확인
...
//    // 로그인
//    public ModelAndView mLogin(UserInfoDTO user, RedirectAttributes redirectAttributes, HttpServletRequest request) {
//        mav = new ModelAndView();
//        System.out.println("아이디 비밀번호 확인 :" + user);
//
//        // 로그인 실패 메시지 초기화
...
//            session.setAttribute("adminName", "admin");
//
//            // 페이지 리디렉션: referer를 확인하여 강아지 또는 고양이 페이지로 리디렉션
//            String referer = request.getHeader("referer");
//            if (referer != null && referer.contains("cat")) {
//                mav.setViewName("redirect:/cat/index");  // 고양이 페이지로 리디렉션
//            } else {
//                mav.setViewName("redirect:/");  // 기본 강아지 페이지로 리디렉션
//            }
//            return mav;
//        }
//
...
//                session.setAttribute("loginNickname", login.getUserNickname());
//
//                // 로그인 성공 후 리디렉션: referer를 확인하여 강아지 또는 고양이 페이지로 리디렉션
//                String referer = request.getHeader("referer");
//                if (referer != null && referer.contains("cat")) {
//                    mav.setViewName("redirect:/cat/index");  // 고양이 페이지로 리디렉션
//                } else {
...
//    }
    ...
    public ModelAndView mLogin(UserInfoDTO user, RedirectAttributes redirectAttributes) {
        ...
        if (entity.isPresent()) {
            ...
            if (pwEnc.matches(user.getUserPw(), entity.get().getUserPw())) {
                ...
                // 로그인 성공 후 referer 확인하여 리디렉션
                mav.setViewName("redirect:/");  // 기본 강아지 페이지로 리디렉션
                return mav;
            } else {
                // 비밀번호 불일치
                System.out.println("비밀번호가 일치하지 않습니다.");
                session.setAttribute("loginError", "password");  // 비밀번호 불일치 메시지 저장
            }
        } else {
            // 아이디 존재하지 않음
            System.out.println("아이디 존재하지 않습니다.");
            session.setAttribute("loginError", "notfound");  // 아이디 없음 메시지 저장
        }

        // 로그인 실패 시 referer 기반으로 리디렉션 처리
        mav.setViewName("redirect:/");  // 기본 강아지 페이지로 리디렉션
        return mav;
    }
    ...
    public void deleteUserAndRelatedData(String loginId) {
        ...
        // 애완동물 삭제
        List<PetEntity> pets = prepo.findByUserId(loginId);
        prepo.deleteAll(pets); // 애완동물 삭제
    ...
    public UserInfoDTO getUserInfo(String userId) {
        UserInfoEntity userEntity = urepo.findById(userId)
...
//    }
    ...
    public List<UserInfoDTO> getAllUsers() {
        List<UserInfoEntity> users = urepo.findAll();
        return users.stream().map(UserInfoDTO::toDTO).collect(Collectors.toList());
    }
}
