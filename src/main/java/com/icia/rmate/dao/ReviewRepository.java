...
//    List<RevEntity> findByRevTitleContainingOrderByREVNumDesc(@Param("keyword") String keyword);
    ...
    @Query("SELECT r FROM RevEntity r WHERE LOWER(r.RevContents) LIKE LOWER(CONCAT('%', :keyword, '%')) ORDER BY r.REVNum DESC")
...
//    void incrementRRp(@Param("RevNum") int RevNum);  // Integer로 매핑 //레파지토리
